...
export async function analyzeInvoice(
  ...
  const prompt = `
    ...
    4. استخراج الأصناف (قاعدة هامة جداً): قم بتصنيف الأصناف الموجودة في الفاتورة ووضعها في الحقول المخصصة لها بدقة.
       - الزيوت: ضع نوع الزيت في oilName، وكميته (عدد اللترات) في oilQty، والسعر الإجمالي لكل اللترات في oilPrice.
       ...
       - **قاعدة السعر الذهبية**: السعر المسجل يجب أن يكون دائماً الإجمالي الكلي للصنف (الكمية × سعر الوحدة)، وليس سعر الوحدة الواحدة.
         مثال: 8 لتر زيت × 40 ريال = سعر الحقل 320 ريال. 4 كفرات × 500 ريال = سعر الحقل 2000 ريال.
       - إذا كانت الفاتورة تعرض سعر الوحدة فقط، فاضربه في الكمية للحصول على الإجمالي.
       ...
       - إذا كان هناك أكثر من صنف في نفس الفئة، اجمع أسمائهم بفاصلة (+) واجمع أسعارهم (مثال: Name: "بوجي + قماش"، Qty: "4 + 1"، Price: "100 + 150").
    ...
    قواعد هامة للإخراج:
    1. إذا كانت البيانات غير موجودة أو غير واضحة، اترك الحقل فارغاً تماماً "" ولا تكتب null أو ####### أو أي رمز آخر.
    2. أسعار الأصناف: يجب أن يكون السعر **الإجمالي الكلي للصنف (الكمية × سعر الوحدة) وقبل الضريبة**.
       - مثال صحيح: 8 لتر × 40 ريال → سجّل 320 ريال (وليس 40).
       - مثال صحيح: 4 كفرات × 500 ريال → سجّل 2000 ريال (وليس 500).
       - إذا كان السعر الإجمالي يشمل الضريبة (15%)، فاقسمه على 1.15.
    3. التاريخ: استخرجه بتنسيق YYYY-MM-DD (مثل 2024-05-12).
    4. الأعمدة المطلوبة في الإكسل هي: ${extraFields.join(' | ')}. تأكد من مطابقة هذه الحقول.
    5. اترك حقل الملاحظات (notes) فارغاً تماماً "".
    6. استخرج رقم اللوحة بدقة "اللوحة".
    7. لا تستخدم كلمة null نهائياً في أي حقل.
